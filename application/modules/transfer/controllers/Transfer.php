...
class Transfer extends CI_Controller {
	...
	function index($offset=0, $order_column='id', $order_type='desc')
	{
		if($this->session->userdata('logged_in'))
		{
			...
			//table data
			$this->template->set('title','Create TO :: List');
			$this->template->load('cpanel/template','transferList',$data);
		}
		else
		{
	...
	function create ()
	{
		...
		// load view
		$this->template->set('title','Create TO :: Create ');
		$this->template->load('cpanel/template','transferAddUpdate',$data);
	}
	function update($id)
	{
		$session_data = $this->session->userdata('logged_in');
